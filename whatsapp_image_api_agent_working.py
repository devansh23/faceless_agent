...
def initialize_whatsapp_session():
    ...
    global playwright_instance, whatsapp_context, whatsapp_page, session_initialized
    ...
    try:
        ...
        whatsapp_context = playwright_instance.chromium.launch_persistent_context(
            user_data_dir, 
            headless=False, 
...
def generate_image():
    """Generate image endpoint - accepts single prompt via API"""
    try:
        ...
        logger.info(f"Received API request: reel={reel_number}, snippet={snippet_number}, prompt={image_prompt[:50]}...")
        
        # Generate single image with the provided prompt
